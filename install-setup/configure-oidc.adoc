...
CAUTION: AI Data Engine Console에 액세스하려면 OIDC를 구성해야 합니다. 구성이 완료되면 모든 인증은 OIDC를 통해 이루어집니다. OIDC가 구성되지 않은 경우 관리자는 물론 데이터 엔지니어와 데이터 과학자도 콘솔에 액세스할 수 없습니다. 이 경우 System Manager에 로그인할 때 로컬 인증 방식이 사용됩니다.
