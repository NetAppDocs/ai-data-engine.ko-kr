...
[role="lead"]
데이터 엔지니어 또는 데이터 과학자가 워크스페이스에서 데이터 컬렉션을 생성하고 게시한 후에 AI Data Engine(AIDE) 클러스터에 대한 해당 컬렉션의 상태, 크기 및 영향에 대한 가시성이 필요합니다.
...
.시작하기 전에
* 데이터 수집을 보려면 ONTAP System Manager에서 _스토리지 관리자_ 권한 또는 AIDE Console(`\https://<cluster_management_ip>/console`에서 _data engineer_ 또는 _data scientist_ 권한이 필요합니다.
* 메타데이터가 성공적으로 추출된 작업 공간이 하나 이상 존재합니다.
* 데이터 엔지니어 또는 데이터 과학자가 AIDE Console에서 최소 하나 이상의 데이터 수집을 생성하고 게시했습니다.
* AI Data Engine 프리미엄 서비스 라이센스가 설치되어 있고 추론 기능이 활성화되어 벡터화 및 검색 엔드포인트가 작동합니다.
